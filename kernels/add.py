...
#Accepts mx1 vectors
@triton.jit
def add_kernel(a_ptr, b_ptr, c_ptr, stride_a, stride_b, stride_c, M, BLOCK_SIZE: tl.constexpr):
...
def add(a: torch.tensor, b: torch.tensor):
    if a.shape != b.shape:
        raise Exception("Attempting to add tensors which are not the same shape")
    ...
    add_kernel[grid](a, b, c, stride_a, stride_b, stride_c, M, BLOCK_SIZE=BLOCK_SIZE)
